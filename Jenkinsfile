pipeline {
    ...
    stages {
      stage('Build/Deploy app to staging') {
        steps {
          sshPublisher(publishers: [sshPublisherDesc(configName: 'staging', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'npm install && yarn start', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '', remoteDirectorySDF: false, removePrefix: '', sourceFiles: '**/*')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
        }
      }
        ...
        steps {
            dir ('/home/usr_2210617_my_ipleiria_pt/jenkins') {
                ...
                dir ('qs_cypress/') {
                  ...
                  sh 'npx cypress run --config baseUrl="localhost:8080"  --config-file cypress_pipeline.json --browser ${BROWSER} --spec ${SPEC} --reporter mochawesome'
